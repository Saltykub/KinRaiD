...
const Question = (props) => {
    ...
    const handleYes = () => {
        const newPick = {pick: 'yes', name: questions[count].name};
        const newCount = count+1;
        setPick(pick.concat(newPick));
        setCount(newCount);
    }
     const handleNo = () => { 
        const newPick = {pick: 'no', name: questions[count].name}
        const newCount = count+1;
        setPick(pick.concat(newPick));
        setCount(newCount);
    }
     const handleMaybe = () => {
        const newPick = {pick: 'maybe', name: questions[count].name};
        const newCount = count+1;
        setPick(pick.concat(newPick));
        setCount(newCount);
    }
    ...
    return (
        ...
        <div className="flex flex-col items-center justify-center sm:flex-row sm:items-center sm:space-x-8">
            ...
            <Stop questions={questions} setCount={setCount}/>
        </div>
